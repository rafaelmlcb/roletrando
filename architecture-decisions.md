...
**Consequência:** Arquitetura robusta para combater trapaças (cheats) e garantir sincronismo perfeito (latência <100ms) em LAN e WAN. O servidor (App Backend) introduziu um sistema de "Lobby" oficial, bloqueando acesso quando a partida inicia. O botão "INICIAR JOGO AGORA" na sala de espera dispara o auto-completamento da sala com robôs caso o número mínimo de pessoas (3) não seja alcançado, o que passou a valer tanto para o `Roletrando` quanto para o `Quiz`. Pavimentada a trilha final para adicionar o multiplayer no *Millionaire*.
